...
__webpack_require__(/*! ./ubication_modal */ "./resources/js/ubication_modal.js");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var modalShoppingCar = document.querySelector('#modalShoppingCar'); // Seleccion los elementos que abren el modal de carrito, y les agrego el evento 'click'
  ...
  document.querySelectorAll('.shoppingCarButtonOpenModal').forEach(function (item) {
    item.addEventListener('click', function (event) {
      ...
      document.getElementById('amountContainerShoppingCar').style.display = 'none'; // Hago visible el modal

      modalShoppingCar.style.display = 'block'; // Hay una tienda seleccionada --- en caso de si, se esta agregando los productos de esta tienda

      var brandSelectedToBuy = localStorage.getItem('brandSelectedToBuy'); //  obtengo el contenedor donde se mostrara la marca seleccionada

      var containerBrandShoppingCar = document.getElementById('containerBrandShoppingCar');
      containerBrandShoppingCar.innerHTML = ''; // Obtengo el componente card de ejemplo

      var cardBrandShoppingCar = document.getElementById('cardBrandShoppingCar');

      if (brandSelectedToBuy !== null) {
        // Llevo el string al formato JSON
        var _brandSelected = JSON.parse(brandSelectedToBuy); // creo un clon del card de ejemplo


        var newCardBrandShoppingCar = cardBrandShoppingCar.firstElementChild.cloneNode(true); // Obtengo los campos del card  clonado

        var newTitle = newCardBrandShoppingCar.querySelector('#titleCardBrandShoppingCar');
        var newImage = newCardBrandShoppingCar.querySelector('#imgCardBrandShoppingCar');
        var newAddress = newCardBrandShoppingCar.querySelector('#addressCardBrandShoppingCar'); // Asigno los valores del local storage a los campos html

        newTitle.textContent = _brandSelected.title;
        newTitle.href = _brandSelected.href;
        newImage.style.backgroundImage = _brandSelected.image;
        newAddress.textContent = _brandSelected.address; // Inserto el card en el modal

        containerBrandShoppingCar.appendChild(newCardBrandShoppingCar); // agrego el nombre de la tienda donde se esta comprando

        document.getElementById('titleBrandInShoppingCar').textContent = _brandSelected.title; // Muestro la tienda

        document.getElementById('brandInShoppingCar').removeAttribute('hidden');
      } // Obtengo los productos del local storage


      var ProductsLocalStorage = localStorage.getItem('productsShoppingCar'); // Hay elementos en el carrito de compras?

      if (ProductsLocalStorage !== null) {
        // Llevo el string al formato JSON
        var _arrayProductsLocalStorage = JSON.parse(ProductsLocalStorage); // oculto y muestro los textos dependiendo de si hay o no productos en el localsotorage


        localStorageProducts(_arrayProductsLocalStorage); //  obtengo el contenedor donde se mostraran todos los productos del carrito
        ...
        _arrayProductsLocalStorage.forEach(function (product) {
          ...
          var newPrice = newCardProductShoppingCar.querySelector('.priceItemInShoppingCar');
          ...
          newImage.src = product.image; // Si hay tienda seleccionada, debo colocar los precios de cada producto en esa tienda

          if (brandSelectedToBuy !== null) {
            newPrice.textContent = "".concat(product.price, " USD$"); // Mostrar el monto total de los productos
      ...
      var deleteBrandDetailInShoppingCar = document.getElementById('deleteBrandDetailInShoppingCar');

      if (deleteBrandDetailInShoppingCar !== null) {
        deleteBrandDetailInShoppingCar.addEventListener('click', function (event) {
          ...
          document.getElementById('compararButton').style.display = 'inline'; // verifico si hay produtos en el carrito
          // Obtengo los productos del local storage

          var ProductsLocalStorage = localStorage.getItem('productsShoppingCar');

          if (ProductsLocalStorage !== null) {
            var _arrayProductsLocalStorage2 = JSON.parse(ProductsLocalStorage);

            if (_arrayProductsLocalStorage2.length < 1) {
              ...
              var productInShoppingCarDOM = document.querySelectorAll('.priceItemInShoppingCar');
              productInShoppingCarDOM.forEach(function (product) {
              ...
              var idProductsBrand = document.querySelectorAll('.idProductBrand');

              if (idProductsBrand.length > 0) {
                // coloco todas las cantidades de los productos a cero, en el DOM
                idProductsBrand.forEach(function (product) {
      ...
      updateAmount(); // Finalizar compra, al dar click en el boton comprar
      ...
      finalizarCompra.addEventListener('click', function (event) {
        ...
        var brandSelectedToBuy = localStorage.getItem('brandSelectedToBuy');
        var brand = JSON.parse(brandSelectedToBuy);

        if (brandSelectedToBuy !== null) {
          // Obtengo los productos del local storage
          var _ProductsLocalStorage = localStorage.getItem('productsShoppingCar');

          var products = JSON.parse(_ProductsLocalStorage);
          var amountSelected = localStorage.getItem('amount');
          var amount = JSON.parse(amountSelected);
          ...
          axios({
            ...
            if (res.data === 0) {
              ...
              localStorage.removeItem('brandSelectedToBuy');
              localStorage.removeItem('productsShoppingCar'); // redirecciono a la vista compras
  ...
  var shoppingCarButtonModalClose = document.querySelector('#shoppingCarButtonModalClose'); // Ocultar el modal de carrito de compras
  ...
  if (addProduct !== null) {
    ...
    addProduct.addEventListener('click', function () {
      // Obtengo los productos del local storage
      var ProductsLocalStorage = localStorage.getItem('productsShoppingCar'); // Obtengo los datos del producto
      ...
      var titleProduct = document.querySelector('#titleProduct').textContent.trim();
      ...
      if (ProductsLocalStorage === null) {
        ...
        newProduct = setNewProduct(idProduct, titleProduct, quantity, 0, srcImageProduct); // Agrego el precio del producto, en caso de tener una tienda seleccionada y estar en la vista de detalles del producto de la tienda
        // obtengo la tienda seleccionada del LocalStorage

        var brandSelectedToBuy = localStorage.getItem('brandSelectedToBuy'); // veo si existe tal tienda

        if (brandSelectedToBuy !== null) {
          var priceProductDetailBrandView = document.getElementById('priceProductDetailBrandView'); // veo si estoy en la vista de detalles del producto de la tienda

          if (priceProductDetailBrandView !== null) {
            var price = priceProductDetailBrandView.textContent;
            newProduct = setNewProduct(idProduct, titleProduct, quantity, price, srcImageProduct);
          } else {
            var quantityAvalaible = document.getElementById('quantityProductDOM').textContent;
            var priceProductDOM = document.getElementById('priceProductDOM');
            var _price = priceProductDOM.textContent;

            if (quantityAvalaible !== '') {
              // El producto esta disponible en esta marca
              newProduct = setNewProduct(idProduct, titleProduct, quantity, _price, srcImageProduct);
            } else {
              // producto no disponible en esta marca
              // eliminao la marca del local storage
              localStorage.removeItem('brandSelectedToBuy');
              document.getElementById('brandInShoppingCar').setAttribute("hidden", true); // notificacion de que ya no se esta comprando en la marca
        ...
        var newProductLocalStorage = '[' + JSON.stringify(newProduct) + ']'; // Muestro el span que indica que hay producots en el carrito
        ...
        console.log('siiiiii'); // Almaceno el producto en el localStorage

        localStorage.setItem('productsShoppingCar', newProductLocalStorage); // El carrito tiene productos
      } else {
        var _arrayProductsLocalStorage3 = JSON.parse(ProductsLocalStorage);

        var productInShppingCar = false; // Recorro el array de prodcutos a ver si el producto ya se encuentra en el carrito

        _arrayProductsLocalStorage3.forEach(function (product) {
        ...
        if (productInShppingCar) {
          ...
          updateTotalProductsShoppingCar(_arrayProductsLocalStorage3);
          shownHideCompareFloatButton(_arrayProductsLocalStorage3);
          updateBrandsPrice(quantity); // Almaceno en local storage

          localStorage.setItem('productsShoppingCar', JSON.stringify(_arrayProductsLocalStorage3)); // El producto no estaba en el carrito de compras, es nuevo!
        } else {
          quantity++;
          quantityProductDetail.textContent = quantity;
          newProduct = setNewProduct(idProduct, titleProduct, quantity, 0, srcImageProduct); // Agrego el precio del producto, en caso de tener una tienda seleccionada y estar en la vista de detalles del producto de la tienda
          ...
          var _brandSelectedToBuy = localStorage.getItem('brandSelectedToBuy'); // veo si existe tal tienda


          if (_brandSelectedToBuy !== null) {
            var _priceProductDetailBrandView = document.getElementById('priceProductDetailBrandView'); // veo si estoy en la vista de detalles del producto de la tienda


            if (_priceProductDetailBrandView !== null) {
              var _price2 = _priceProductDetailBrandView.textContent;
              newProduct = setNewProduct(idProduct, titleProduct, quantity, _price2, srcImageProduct);
            } else {
              // Estoy en la vista de detalles del producto, vista general
              // verifico si la tienda tiene el producto en su inventario
              var _quantityAvalaible = document.getElementById('quantityProductDOM').textContent;

              var _priceProductDOM = document.getElementById('priceProductDOM');

              var _price3 = _priceProductDOM.textContent;

              if (_quantityAvalaible !== '') {
                // El producto esta disponible en esta marca
                newProduct = setNewProduct(idProduct, titleProduct, quantity, _price3, srcImageProduct);
              } else {
                // producto no disponible en esta marca
                // eliminao la marca del local storage
                localStorage.removeItem('brandSelectedToBuy');
          ...
          _arrayProductsLocalStorage3.push(newProduct); // Muestro el span que indica que hay producots en el carrito


          updateTotalProductsShoppingCar(_arrayProductsLocalStorage3);
          shownHideCompareFloatButton(_arrayProductsLocalStorage3);
          updateBrandsPrice(quantity); // Agrego el precio del producto, en caso de tener una tienda seleccionada y estar en la vista de detalles del producto de la tienda
          // Almaceno el producot en el localStorage

          localStorage.setItem('productsShoppingCar', JSON.stringify(_arrayProductsLocalStorage3));
        }
      }
    }, {
      passive: true
  ...
  if (subtractProduct !== null) {
    ...
    subtractProduct.addEventListener('click', function () {
      ...
      if (quantity > 0) {
        ...
        if (ProductsLocalStorage !== null) {
          var _arrayProductsLocalStorage4 = JSON.parse(ProductsLocalStorage);
          ...
          _arrayProductsLocalStorage4.forEach(function (product) {
            if (idProduct == product.id) {
              productInShppingCar = true;
              quantity--;
              quantityProductDetail.textContent = quantity;
          ...
          if (quantity == 0) {
            ...
            _arrayProductsLocalStorage4.splice(keyArray, 1);
          } // Muestro el span que indica que hay producots en el carrito


          updateTotalProductsShoppingCar(_arrayProductsLocalStorage4);
          shownHideCompareFloatButton(_arrayProductsLocalStorage4);
          updateBrandsPrice(quantity); // Guardo en localstorage

          localStorage.setItem('productsShoppingCar', JSON.stringify(_arrayProductsLocalStorage4));
        }
      }
    }, {
      passive: true
  ...
  document.querySelectorAll('.shoppingCarButtonOpenModal').forEach(function (item) {
    item.addEventListener('click', function (event) {
      ...
      setQuantiyFromShoppingCar('addProductModalShoppingCard', 'add'); // Reducir la cantidad de productos en el modal de carrito de comrpas, desde el modal del carrito

      setQuantiyFromShoppingCar('substractProductModalShoppingCard', 'substract'); // Eliminar prodcuto del modal de carrito de compras, con boton de elminar que esta en el modal del carrito

      document.querySelectorAll('.removeProductModalShoppingCar').forEach(function (item) {
        item.addEventListener('click', function (event) {
          // Obtengo los productos del local storage
          var ProductsLocalStorage = localStorage.getItem('productsShoppingCar');
          var arrayProductsLocalStorage = JSON.parse(ProductsLocalStorage); // En que elemento se dio click? en el spna, en el svg o en el path?
          ...
          if (productInShppingCar) {
            ...
            if (_quantityProductDetail !== null) {
              _quantityProductDetail.textContent = 0;
            ...
            if (brandSelectedToBuy !== null) {
              var brand = JSON.parse(brandSelectedToBuy); // Obtengo los productos del DOM

              var productsIdDOM = document.querySelectorAll('.idProductBrand');
              productsIdDOM.forEach(function (product) {
          ...
          updateBrandsPrice(0); // Almaceno el producot en el localStorage
  ...
  var ProductsLocalStorage = localStorage.getItem('productsShoppingCar'); // Transformo el string a array

  var arrayProductsLocalStorage = JSON.parse(ProductsLocalStorage);
  ...
  document.querySelectorAll('.idProductCard').forEach(function (product) {
    var idProduct = product.textContent;
  ...
  var backgroundModalShoppingCar = document.getElementById('backgroundModalShoppingCar');
  backgroundModalShoppingCar.addEventListener('click', function (event) {
...
function localStorageProducts(arrayProductsLocalStorage) {
  ...
  if (arrayProductsLocalStorage.length > 0) {
    ...
    if (brandSelectedToBuy === null) {
      ...
      var enlaceConBrand = document.getElementById('enlaceConBrand'); // enlaceConBrand.querySelector('a');
...
function shownHideCompareFloatButton(arrayProductsLocalStorage) {
  ...
  if (arrayProductsLocalStorage == 0) {
    ...
    compareFloatButton.classList.remove("shownButton"); // Mostrar el monto total de los productos
...
function updateAmount() {
  ...
  if (brandSelectedToBuy !== null) {
    ...
    var amountShoppingCar = document.getElementById('amountShoppingCar');
    amountShoppingCar.textContent = "".concat(amount, " USD$"); // guardar el monto en el localStorage
...
function setQuantiyFromShoppingCar(editButton, typeSet) {
  ...
  document.querySelectorAll('.' + editButton).forEach(function (item) {
    item.addEventListener('click', function (event) {
      // Obtengo los productos del local storage
      var ProductsLocalStorage = localStorage.getItem('productsShoppingCar');
      var arrayProductsLocalStorage = JSON.parse(ProductsLocalStorage); // En que elemento se dio click? en el spna, en el svg o en el path?
      ...
      if (quantity > 0 && typeSet == 'substract' || typeSet == 'add') {
        ...
        var id_product_DOM = document.getElementById('idproduct');
        ...
        rootContainer.querySelector('.quantityCardProductShoppingCar').textContent = quantity; // El producto llego a cero? se debe eliminar del local storage
        ...
        updateTotalProductsShoppingCar(arrayProductsLocalStorage);
        shownHideCompareFloatButton(arrayProductsLocalStorage); // actualizo la cantidad en el DOM de la vista de tienda, en caso se estar en la vista de detalles de la tienda seleccionada

        updateQuantityProductsInBrandView(idProduct, quantity); // Almaceno el producot en el localStorage
...
function updateBrandsPrice(quantity) {// const priceBrandsInProdcutDetailView = localStorage.getItem('priceBrandsInProdcutDetailView')
...
/***/ "./resources/js/ubication_modal.js":
/*!*****************************************!*\
  !*** ./resources/js/ubication_modal.js ***!
  \*****************************************/
/***/ (() => {
...
document.addEventListener("DOMContentLoaded", function () {
  var modalUbication = document.getElementById('modalUbication');
  var ubication = localStorage.getItem('ubication');

  if (ubication === null) {
    modalUbication.style.display = 'block';
  } else {
    var ubicationObject = JSON.parse(ubication);
    var city = ubicationObject.city;

    if (city !== null) {
      // Agregar la ciudad en el DOM
      var ubicationText = document.querySelectorAll('.ubicationText');
      ubicationText.forEach(function (element) {
  ...
  var formUbication = document.getElementById('formUbication');
  formUbication.addEventListener('submit', function (event) {
    var selectState = document.getElementById('selectState');
    var selectCity = document.getElementById('selectCity');
    var state_id = selectState.value;
    var state = selectState.selectedOptions.item(0).text;
    var city_id = selectCity.value;
    var city = selectCity.selectedOptions.item(0).text;
    ...
    localStorage.setItem('ubication', JSON.stringify(objectUbicationLocalStorage)); // Agregar la ciudad en el DOM
    ...
    var url = window.location.pathname; // Pregunto si estoy en una vista donde se requiere actualizar la vista
  ...
  var selectState = document.getElementById('selectState');
  selectState.addEventListener('change', function (event) {
    var state_id = selectState.value;
    var selectCity = document.getElementById('selectCity');
  ...
  var ubicationTag = document.querySelectorAll('.ubicationTag');
  ubicationTag.forEach(function (element) {
    element.addEventListener('click', function (event) {
      modalUbication.style.display = 'block';
    });
  });
  var modalUbicationButtonClose = document.getElementById('modalUbicationButtonClose');
  modalUbicationButtonClose.addEventListener('click', function (event) {
    modalUbication.style.display = 'none';
  }); // cerrar modal al dar click fuera del modal

  var contentModalUbication = document.getElementById('contentModalUbication');
  var backgroundModalUbication = document.getElementById('backgroundModalUbication');
  backgroundModalUbication.addEventListener('click', function (event) {
