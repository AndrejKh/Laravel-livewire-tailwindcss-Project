        ...
        <div class="grid grid-cols-1 md:grid-cols-2 max-w-7xl w-full shadow rounded-md p-6 md:px-10 bg-white">
            @foreach ($principal_categories as $category)
                <div class="text-gray-900 font-bold text-lg my-4 px-1">
                    <a class="hover:text-blue-600 mb-3" href="{{route('products.category.show', $category->slug)}}" aria-label="ver productos de la categoría {{ $category->category }}">
                        {{ $category->category }}
                        ...
                        @foreach ($categories_children as $category_child)
                            @if ($category->id == $category_child->padre_id)
                                <div class="col-span-1 pl-2">
                                    <a class="text-sm text-gray-500 hover:text-blue-800" href="{{route('products.category.show', $category_child->slug)}}" aria-label="ver productos de la subcategoría {{ $category_child->category }}">
