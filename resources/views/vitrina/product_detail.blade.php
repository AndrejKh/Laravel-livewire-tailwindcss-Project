...
@section('content')
        ...
        <div class="max-w-7xl w-full px-2">
            <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
                ...
                <div class="max-w-7xl col-span-1 md:col-span-3 rounded-xl shadow-md overflow-hidden relative h-44 md:h-auto" style="background-color:#fff!important;">
                    <img class="h-full mx-auto" src="/storage/{{ $product->photo_main_product }}" alt="{{ $product->title }}" id="srcImageProduct" width="300px" height="300px" loading="lazy">
    ...
    <div class="my-1 md:my-4">
        ...
        <div class="max-w-7xl w-full px-2 mx-auto">
            <span class="text-gray-700 text-sm">Podrás comparar tu carrito de compras al final</span>
        </div>
    </div>
    ...
    <div class="flex justify-center my-4">
        <a class="bg-gray-900 text-white py-2 px-8 rounded-md shadow-md" href="{{ url()->previous() }}" aria-label="seguir agregando al carrito">Seguir agregando al carrito</a>
    </div>
    ...
    <div class="block max-w-7xl w-full px-2 mx-auto mb-10 mt-3">
        ...
        <p class="text-gray-700 mt-4 text-base md:text-lg">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // guardo en local storage los precios e ids, de las marcas, para luego cambiar dinamiecamente los precios cuando se cambie la cantidad
            const priceBrandInProductDetailView = document.querySelectorAll('.priceBrandInProductDetailView');
            let arrayPricesBransProducts = new Array();
            priceBrandInProductDetailView.forEach(element => {
                let id_brand = element.parentElement.parentElement.parentElement.querySelector('.idBrandInPrductDetailView').textContent
                newPrice = {
                    id: id_brand,
                    price: parseInt(element.textContent)
                }
                arrayPricesBransProducts.push(newPrice)
            });
            localStorage.setItem('priceBrandsInProdcutDetailView', JSON.stringify(arrayPricesBransProducts));


            // Actualizo los precios de los abastos, dependiendo si la cantidad es mayor q 1
            const quantityProductDetail = document.getElementById('quantityProductDetail');
            if( quantityProductDetail.textContent > 1 ){
                const priceBrandsInProdcutDetailView = localStorage.getItem('priceBrandsInProdcutDetailView')
                priceBrandsInProdcutDetailView.forEach(element => {
