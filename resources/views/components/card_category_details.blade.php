<div class="grid grid-cols-1 lg:grid-cols-2 gap-1 lg:gap-2 rounded-md overflow-hidden shadow-md my-2 bg-white w-full">
    <div class="col-span-1 lg:col-span-1 px-3 md:px-5 py-2 lg:py-3 text-left">
            <a class="text-gray-900 hover:text-gray-700 text-left font-bold text-lg" href="{{route('products.category.show', $category->slug)}}" aria-label="ver los precios de la categoría {{ $product->category->category }}">
                {{ $category->category }}
            </a>
    ...
    <a class="col-span-1 lg:col-span-1 order-first lg:order-last overflow-hidden" href="{{route('products.category.show', $category->slug)}}" aria-label="ver los precios de la categoría {{ $product->category->category }}">
